...
@section('title', isset($role) ? 'Edit Role - Retribusi Sampah Kudus' : 'Tambah Role Baru - Retribusi Sampah Kudus')
@section('page-title', isset($role) ? 'Edit Role & Hak Akses' : 'Tambah Role Baru')
    ...
    <form action="{{ isset($role) ? route('admin.roles.update', $role->id) : route('admin.roles.store') }}" method="POST">
        ...
        <div style="display: flex; gap: 20px; margin-bottom: 20px;">
            <div style="flex: 1;">
                ...
                <input type="text" name="display_name" value="{{ old('display_name', $role->display_name ?? '') }}" required style="width: 100%; padding: 10px; border: 1px solid #d1d5db; border-radius: 6px;" placeholder="Contoh: Petugas Penagih Lapangan">
            ...
            <div style="flex: 1;">
                ...
                <input type="text" name="name" value="{{ old('name', $role->name ?? '') }}" {{ isset($role) && $role->is_system ? 'readonly' : 'required' }} style="width: 100%; padding: 10px; border: 1px solid #d1d5db; border-radius: 6px; {{ isset($role) && $role->is_system ? 'background:#f3f4f6;' : '' }}" placeholder="Contoh: petugas_lapangan">
        ...
        <div style="margin-bottom: 30px;">
            ...
            <textarea name="description" rows="2" style="width: 100%; padding: 10px; border: 1px solid #d1d5db; border-radius: 6px;">{{ old('description', $role->description ?? '') }}</textarea>
        ...
        <div style="margin-top: 30px; display: flex; justify-content: flex-end; gap: 10px;">
            ...
            <button type="submit" style="background: #2563eb; color: white; border: none; padding: 10px 20px; border-radius: 6px; font-weight: 500; cursor: pointer;">Simpan Role</button>
        </div>
    </form>
</div>
@endsection
